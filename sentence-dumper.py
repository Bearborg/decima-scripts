...
def sentence_representer(dumper: yaml.Dumper, sent: decima.SentenceResource):
    return dumper.represent_str(sent.name)


yaml.add_representer(decima.SentenceResource, sentence_representer)
...
def dump_sentences(filename):
    ...
    with open(filename + '.txt', 'w', encoding='utf8') as out_file:
        out_file.write(out)
        ...
        for f in filenames:
            if os.stat(os.path.join(root, f)).st_size > 0:  # Ignore empty files
                if f == "simpletext.core":
                    pass
                    # TODO
                    # text = read_simple_text_file(open(os.path.join(root, f), 'rb'))
                    # open(os.path.join(root, f + '.txt'), 'w').write(text)
